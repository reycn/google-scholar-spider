...
def google_scholar_spider(GoogleScholarConfig: GoogleScholarConfig):
    ...
    data_ranked = process_data(
        data, GoogleScholarConfig.end_year, GoogleScholarConfig.sortby
    )

    # Plot by citation number
    if GoogleScholarConfig.plot_results:
        plot_results(
            data_ranked.index, data_ranked["Citations"], GoogleScholarConfig.keyword
        )

    # Save results
    if GoogleScholarConfig.save_csv:
        save_data_to_csv(
            data_ranked, GoogleScholarConfig.csvpath, GoogleScholarConfig.keyword
        )
...
def create_main_url(GoogleScholarConfig: GoogleScholarConfig) -> str:
    if GoogleScholarConfig.start_year:
        gscholar_main_url = GSCHOLAR_URL + STARTYEAR_URL.format(
            GoogleScholarConfig.start_year
        )
    else:
        gscholar_main_url = GSCHOLAR_URL

    if GoogleScholarConfig.end_year != current_year:
        gscholar_main_url = gscholar_main_url + ENDYEAR_URL.format(
            GoogleScholarConfig.end_year
        )
...
def fetch_data(
    GoogleScholarConfig: GoogleScholarConfig,
    session: requests.Session,
    gscholar_main_url: str,
    pbar: None,
) -> pd.DataFrame:
    links: List[str] = []
    title: List[str] = []
    ...
    for n in range(0, GoogleScholarConfig.nresults, 10):
        ...
        url = gscholar_main_url.format(
            str(n), GoogleScholarConfig.keyword.replace(" ", "+")
        )
        if GoogleScholarConfig.debug:
    ...
    # Create a dataset
    data = pd.DataFrame(
        list(zip(author, title, citations, year, publisher, venue, describe, links)),
        index=rank[1:],
        columns=[
            "Author",
            "Title",
